...
import { getSidebarProjectColorTheme, sameSidebarProjectRoot } from "./utils/sidebarProjects.js"
import { resolvePresentationMode } from "./utils/presentationMode.js"
import {
...
const DmuxApp: React.FC<DmuxAppProps> = ({
  ...
  const getSettingsManagerForProjectRoot = React.useCallback((targetProjectRoot: string) => {
  ...
  const getAvailableAgentsForProject = (targetProjectRoot: string = selectedProjectRoot) =>
    resolveEnabledAgentsSelection(
      getSettingsManagerForProjectRoot(targetProjectRoot).getSettings().enabledAgents
    )
  ...
  useEffect(() => {
    const checkHooksPreference = async () => {
      ...
      const settings = getSettingsManagerForProjectRoot(sessionProjectRoot).getSettings()
  ...
  const resolveProjectThemeName = React.useCallback((activeProjectRoot: string) => {
    return getSidebarProjectColorTheme(sidebarProjects, activeProjectRoot)
      || getSettingsManagerForProjectRoot(activeProjectRoot).getSettings().colorTheme
      || DEFAULT_DMUX_THEME
  }, [getSettingsManagerForProjectRoot, sidebarProjects])
  ...
  const refreshDmuxSettings = (_activeProjectRoot: string = selectedProjectRoot) => {
    setSettings(getSettingsManagerForProjectRoot(_activeProjectRoot).getSettings())
